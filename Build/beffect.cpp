...
//
//#define	g_aBeffect[nCntBeffect].fSizeX		(1.0f)		// 頂点サイズ
//#define	g_aBeffect[nCntBeffect].fSizeY		(1.0f)		// 頂点サイズ
#define	VALUE_MOVE_BEFFECT	(5.0f)		// 移動速度

#define	MAX_BEFFECT		(1024)		// パーティクル最大数
...
static BEFFECT					g_aBeffect[MAX_BEFFECT];		// パーティクルワーク
...
HRESULT InitBeffect(void)
{
	...
	GetDevice()->CreateShaderResourceView(
		mpBeffectBuffer,          // アクセスするテクスチャ リソース
